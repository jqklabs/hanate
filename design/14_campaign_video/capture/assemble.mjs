...
const EDIT = [
  ...
  { id: 'H3a', kind: 'plate', use: 2.2, trans: 'fade', transDur: 0.18,
    cam: { z: [1.00, 1.00], ease: 'linear' } },
  /* ② 일갈 + 마무리 포즈. 컷 전환 없이 **이 실루엣에 붙어 있는다.**
     방자는 역광 실루엣이라 이목구비가 안 읽힌다(R13 완화안). 카메라 고정.
     원작 분석의 3단계가 한 클립에 다 있다:
  ...
  /* 로고 — **화투를 판에 내리치듯.** 카메라 배율로 구현돼 있다(구워진 게 아니다).
     한 컷 안에서 떨어뜨리면 착지 순간을 정확히 못 잡는다 → 두 컷으로 나눈다.
...
const EASE = {
  ...
  /* 화투를 내리치듯 — 위에서 뜸을 들이다 **가속하며** 내리꽂힌다.
     dash 같은 ease-out은 반대로 처음에 확 왔다가 끝에서 느려져
     '떨어진다'가 아니라 '내려앉는다'로 읽혔다. 착지는 끝에서 일어나야 한다. */
  drop:   (P) => `(${P}*${P}*${P})`,
};
...
/* decay/freq: 흔들림의 성격. 기본(11/38)은 묵직한 '쿵'이고,
   값을 키우면(26/62) 짧고 날카로운 '챡' — 화투를 내리치는 소리에 가깝다. */
function impact({ shake = 0, flash = 0, at: t0 = 0,
                  decay = 11, freq = 38 } = {}, W = 0, H = 0) {
  let f = '';
  if (shake) {
    /* t0: 충격이 오는 시각. 착지하는 순간에 흔들려야 '쾅'으로 읽힌다 —
       컷 시작에 흔들면 이미 멈춘 화면이 뒤늦게 떠는 꼴이 된다. */
    const A = shake, D = decay, F = freq;
  ...
  /* 플래시도 착지 시각(t0)에 맞춘다. 예전엔 컷 시작에서 터져
     떨어지기도 전에 화면이 번쩍했다. */
  if (flash)
    f += `,eq=brightness='${flash}*gte(t\,${t0})*exp(-max(0\,t-${t0})*16)':eval=frame`;
  return f;
}
