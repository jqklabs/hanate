...
제목과 CTA는 붓글씨 이미지, 부제는 게임 폰트로 찍는다.
  로고 : gen3/logo_og.png     (금박 붓글씨, 12장 공용)
  CTA  : gen3/tcta/type.png   (금박 붓글씨, 12장 공용)
  부제 : 게임 폰트 SSRockRegular — 광고에서 본 글씨 = 게임에서 볼 글씨
...
CTA_W_RATIO = 0.52     # CTA 폭 = 로고 폭 × 이 값
...
logo = fit(piece(os.path.join(G, "logo_ink.png" if INK else "logo_og.png")), LOGO_W)
cta  = fit(piece(os.path.join(G, "cta_verm.png") if INK else os.path.join(G, "tcta", "type.png")),
...
gap1, gap2 = round(LOGO_W * 0.055), round(LOGO_W * 0.055)
